...
export function Discussions({
  ...
  const setActiveGroup = (id: string | null) =>
    updateState({ activeGroup: id });
  const isTest = state.currentUser === "test";
  const isSMS =
    state.activeGroup?.startsWith("sms-") ||
    state.activeGroup?.startsWith("sms_");
  const typingTimeoutRef = useRef<NodeJS.Timeout | null>(null);
  const [showRestrictedPopup, setShowRestrictedPopup] = useState(false);
  const [sessionLastRead, setSessionLastRead] = useState<
    Record<string, string>
  >({});
  const [sessionEnterTime, setSessionEnterTime] = useState<
    Record<string, string>
  >({});
  ...
  useEffect(() => {
    ...
    const unsub = onSnapshot(
      doc(db, roomPath, state.activeGroup),
      (snapshot) => {
        if (snapshot.exists()) {
  ...
  const typingNames = typingUserIds.map(
    (uid: string) => state.users[uid]?.name || "Quelqu'un",
  );
  ...
  const handleTyping = async () => {
    ...
    await updateDoc(roomRef, {
      isTyping: arrayUnion(state.currentUser as string),
    }).catch(() => {});

    if (typingTimeoutRef.current) clearTimeout(typingTimeoutRef.current);
    typingTimeoutRef.current = setTimeout(async () => {
      await updateDoc(roomRef, {
        isTyping: arrayRemove(state.currentUser as string),
      }).catch(() => {});
    }, 3000);
  };
  const currentUser =
    isTest || !state.currentUser
      ? null
      : state.currentUserData || state.users[state.currentUser as string];
  ...
  const allRecentMessages = [
    ...Object.values(state.groups || {})
      .filter((g) => {
        ...
        return (
          (g.type === "public" ||
            (g.members && g.members.includes(state.currentUser as string)) ||
            currentUser?.isAdmin ||
            currentUser?.isGrandAdmin ||
            currentUser?.isSuperAdmin) &&
          !g.id.startsWith("sms-dj-help-") &&
          (!g.id.startsWith("sms_dj_bot_") || !isTest)
        );
      })
      .map((g) => {
        const lastRead = currentUser?.lastReadTimestamps?.[g.id] || "0";
        const newCount = (g.messages || []).filter(
          (m) => !m.isSystem && m.timestamp > lastRead,
        ).length;
        const lastMsg = (g.messages || [])
          .filter((m) => !m.isSystem)
          .slice(-1)[0];
    ...
    ...Object.values(state.privateMessages || {})
      ...
      .map((chat) => {
        const otherId =
          chat.members?.find((m: string) => m !== state.currentUser) ||
          chat.members[0];
        const otherName =
          state.users[otherId || ""]?.name || otherId || "Inconnu";
        const lastRead = currentUser?.lastReadTimestamps?.[chat.id] || "0";
        const newCount = (chat.messages || []).filter(
          (m) => !m.isSystem && m.timestamp > lastRead,
        ).length;
        const lastMsg =
          (chat.messages || []).filter((m) => !m.isSystem).slice(-1)[0] ||
  ...
  const [deleteOptionsPrompt, setDeleteOptionsPrompt] = useState<{
    msgIds: string[];
    isMine: boolean;
    isCreator: boolean;
    isSubAdmin: boolean;
  ...
  const [staffDeletePrompt, setStaffDeletePrompt] = useState<string | null>(
    null,
  );
  const [showDeleteSmsPrompt, setShowDeleteSmsPrompt] = useState<string | null>(
    null,
  );
  ...
  const stickers = [
    ...
    {
      id: "party",
      url: "https://api.dicebear.com/7.x/fun-emoji/svg?seed=party",
    },
    ...
    {
      id: "laugh",
      url: "https://api.dicebear.com/7.x/fun-emoji/svg?seed=laugh",
    },
    ...
    {
      id: "music",
      url: "https://api.dicebear.com/7.x/fun-emoji/svg?seed=music",
    },
    {
      id: "vinyl",
      url: "https://api.dicebear.com/7.x/fun-emoji/svg?seed=vinyl",
    },
    {
      id: "headphones",
      url: "https://api.dicebear.com/7.x/fun-emoji/svg?seed=headphones",
    },
    {
      id: "speaker",
      url: "https://api.dicebear.com/7.x/fun-emoji/svg?seed=speaker",
    },
    {
      id: "dance",
      url: "https://api.dicebear.com/7.x/fun-emoji/svg?seed=dance",
    },
  ];
  ...
  useEffect(() => {
    if (activeGroup && state.currentUser && !isTest) {
      ...
      setSessionLastRead((prev) => ({
        ...prev,
        [activeGroup]:
          state.currentUserData?.lastReadTimestamps?.[activeGroup] || "0",
      }));
  ...
  useEffect(() => {
    ...
    if (activeGroup && state.currentUser && !isTest) {
      setTimeout(() => {
        const nowISO = new Date().toISOString();
        const groupMessages =
          state.groups?.[activeGroup]?.messages ||
          state.privateMessages?.[activeGroup]?.messages ||
          [];
        const lastMsgTimestamp =
          groupMessages.length > 0
            ...
            : "0";
        const readTimestamp =
          nowISO > lastMsgTimestamp ? nowISO : lastMsgTimestamp;
        ...
        updateState((prev: AppState) => {
          ...
          if (prev.currentUser && newUsers[prev.currentUser]) {
            ...
            if (newCurrentUserData) {
              newCurrentUserData = {
                ...newCurrentUserData,
                lastReadTimestamps: user.lastReadTimestamps,
              };
            }
          }
    ...
    state.groups?.[activeGroup || ""]?.messages?.length,
    state.privateMessages?.[activeGroup || ""]?.messages?.length,
    activeGroup,
  ]);
  ...
  const handleSendMessage = async (e: React.FormEvent) => {
      ...
      updateState((prev: AppState) => {
        ...
        if (newGroups[activeGroup]) {
          newGroups[activeGroup] = {
            ...newGroups[activeGroup],
            messages: [
              ...(newGroups[activeGroup].messages || []),
              optimisticMsg,
            ],
          };
        }
    ...
    try {
      ...
      if (isSMS) {
        ...
        // Check permissions
        if (group?.banned?.includes(state.currentUser as string))
          return showToast("Tu es banni de ce groupe.");
        if (group?.muted?.includes(state.currentUser as string))
          return showToast("Tu es muet dans ce groupe.");

        const isAdmin =
          group?.admins?.includes(state.currentUser as string) ||
          group?.subAdmins?.includes(state.currentUser as string) ||
          currentUser?.isAdmin ||
          currentUser?.isGrandAdmin ||
          currentUser?.isSuperAdmin;
        const isCreator = group?.creator === state.currentUser;
        const isSubAdmin = group?.subAdmins?.includes(
          state.currentUser as string,
        );
        const isExemptFromGroupLimits =
          isCreator || isSubAdmin || currentUser?.isSuperAdmin;

        if (!(group?.allowOthersToSpeak ?? true) && !isExemptFromGroupLimits) {
          return showToast(
            "Seuls les admins du groupe ou les super admins peuvent parler ici.",
          );
        }
      ...
      if (error.code === "unavailable") {
        showToast(
          "Mode hors-ligne: Le message sera envoyé dès le retour de la connexion.",
        );
      } else {
        showToast(
  ...
  const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    for (let i = 0; i < files.length; i++) {
      ...
      if (file.size > 20 * 1024 * 1024 * 1024) {
        showToast(`Le fichier ${file.name} est trop volumineux (max 20 Go).`);
        continue;
      }
    ...
    try {
      ...
      const text =
        files.length > 1
          ...
          : typeIcons[firstFileType] || "📁 Fichier";
      ...
      const msgData = {
        ...
        groupName: isSMS
          ? otherUser
          : state.groups?.[activeGroup]?.name || "Groupe",
        groupType: isSMS
          ? "SMS"
          : state.groups?.[activeGroup]?.type === "public"
            ? "PUBLIC"
            : "PRIVÉ",
      };
  ...
  const handleVote = async (messageId: string, optionId: string) => {
    ...
    try {
      ...
      try {
        ...
        return showToast(
          "Impossible de voter : connexion à la base de données échouée.",
        );
      }
    ...
    return parts.map((part, i) => {
      if (part.match(urlRegex)) {
        return (
          ...
          >
            <span className="dj-style-text font-black text-[0.95em] break-all underline decoration-2 underline-offset-2">
              {part}
            </span>
          </a>
        );
      ...
      safeText = parseFormatting(safeText);
  ...
  const handleStartThreadSelection = () => {
    ...
    if (isSMSIn)
      return showToast("Les fils ne sont dispo que dans les groupes.");
    ...
    return (
      <div className="bg-white p-8 rounded-[2.5rem] shadow-2xl border border-gray-100 animate-in slide-in-from-bottom-8 duration-500 max-w-md mx-auto">
        <div className="flex justify-between items-center mb-6">
          <h3
            className={`text-xl font-black uppercase tracking-tighter ${djStyleText}`}
          >
            Créer un groupe {activeTab === "public" ? "Public" : "Privé"}
          </h3>
          <button
            onClick={() => {
              setShowCreateGroup(false);
        ...
        <div className="mb-8">
          ...
          <div className="w-full bg-gray-100 rounded-full h-3 overflow-hidden shadow-inner">
            <div
              className={`${djStyleBg} h-full transition-all duration-500 ease-out`}
        ...
        {wizardStep === 1 && (
          <div className="space-y-6 animate-in fade-in slide-in-from-right-4">
            ...
            <button
              onClick={() =>
                newGroupName.trim() ? setWizardStep(2) : showToast("Nom requis")
              }
              className={`w-full py-4 rounded-2xl font-black uppercase tracking-widest text-white shadow-xl hover:scale-[1.02] transition-all active:scale-95 ${djStyleBg}`}
            >
              Continuer
            </button>
          </div>
          ...
          <div className="space-y-6 animate-in fade-in slide-in-from-right-4">
              ...
              <div className="grid grid-cols-1 gap-2">
                {reasons.map((r) => (
                  ...
                  >
                    {r.label}
                    {newGroupReason === r.id && (
                      <div className="w-2 h-2 rounded-full bg-[#0D98BA]" />
                    )}
                  </button>
                ))}
              </div>
            ...
            <div className="flex gap-3">
              <button
                ...
                className="flex-1 py-4 rounded-2xl bg-gray-100 text-gray-500 font-black uppercase tracking-widest text-xs hover:bg-gray-200 transition-all"
              >
                Retour
              </button>
              <button
            ...
            <div>
              ...
              <div className="max-h-60 overflow-y-auto space-y-2 pr-2 custom-scrollbar">
                {Object.keys(state.users || {})
                  .filter((u) => u !== state.currentUser && u !== "DJ_Bot")
                  .map((u) => (
                    <label
                      key={u}
                      className={`flex items-center justify-between p-4 rounded-2xl border-2 transition-all cursor-pointer ${newGroupInvite.includes(u) ? "border-[#0D98BA] bg-blue-50" : "border-gray-50 hover:border-gray-100"}`}
                    >
                      <div className="flex items-center gap-3">
                        <div className="w-8 h-8 rounded-full bg-gray-200 flex items-center justify-center font-bold text-gray-500 text-xs">
                          {state.users[u].avatar ? (
                            <img
                              src={state.users[u].avatar!}
                      ...
                      <input
                        ...
                        onChange={(e) =>
                          e.target.checked
                            ? setNewGroupInvite([...newGroupInvite, u])
                            : setNewGroupInvite(
                                newGroupInvite.filter((i) => i !== u),
                              )
                        }
                      />
                    </label>
              ...
              <button
                onClick={() => {
                  if (newGroupCode.length < 5 || newGroupCode.length > 8) {
                    return showToast(
                      "Le code doit faire entre 5 et 8 caractères.",
                    );
                  }
                  if (
                    !/^(?=.*[a-z])(?=.*[A-Z])(?=.*\d)(?=.*[^A-Za-z0-9]).{5,8}$/.test(
                      newGroupCode,
                    )
                  ) {
                    return showToast(
                      "Mélangez obligatoirement majuscules, minuscules, chiffres et caractères spéciaux.",
                    );
                  }
                  if (
                    Object.values(state.groups || {}).some(
                      (g) => g.code === newGroupCode,
                    )
                  ) {
  ...
  const handleJoinPrivateGroup = async () => {
    ...
    const group = Object.values(state.groups || {}).find(
      (g) => g.type === "private" && g.code === cleanJoinCode,
    );
    if (!group) return showToast("Code introuvable.");
    if ((group.members || []).includes(state.currentUser as string))
      return showToast("Déjà membre.");

    // Check ban
    const banInfo = group.banHistory?.[state.currentUser as string];
    if (banInfo) {
      if (banInfo.count >= 5)
        return showToast("Tu es banni définitivement de ce groupe.");
      if (banInfo.until && new Date(banInfo.until) > new Date()) {
        const remaining = Math.ceil(
          (new Date(banInfo.until).getTime() - new Date().getTime()) /
            (1000 * 60 * 60 * 24),
        );
  ...
  const handleStartSMS = async (otherUid: string) => {
    ...
    try {
      ...
      try {
        ...
        console.error(
          "Erreur lors de la vérification de la discussion privée:",
          e,
        );
      ...
      if (!snap.exists()) {
        try {
          ...
          console.error(
            "Erreur lors de la création de la discussion privée:",
            e,
          );
          setActiveGroup(smsId);
          setSmsSearch("");
  ...
  const handleDeleteMessage = async (
    msgIds: string[],
    option: "me" | "everyone" | "bubble" = "me",
  ) => {
    if (!activeGroup) return;
    const isSMS = activeGroup.startsWith("sms_");
    const path = isSMS ? "private_messages" : "groups";
    const group =
      state.groups[activeGroup] || state.privateMessages[activeGroup];
    if (!group) return;
    ...
    try {
      ...
      if (option === "me") {
        showToast(
          count > 1
            ? `${count} messages supprimés pour vous.`
            : "Message supprimé pour vous.",
        );
      } else if (option === "everyone") {
        showToast(
          count > 1
            ? `${count} messages supprimés pour tous.`
            : "Message supprimé pour tous.",
        );
      } else if (option === "bubble") {
        showToast(
          count > 1 ? `${count} bulles supprimées.` : "Bulle supprimée.",
        );
      }
    } catch (error) {
  ...
  const handleToggleSubAdmin = async (userId: string) => {
    ...
    if (
      ...
      return showToast(
        "Seul l'admin du groupe ou le Staff peut nommer des sous-admins.",
      );
  ...
  const confirmDeleteAll = async () => {
    ...
    try {
      ...
      const group =
        state.groups[activeGroup] || state.privateMessages[activeGroup];
      const messagesToDelete = (group.messages || []).filter(
        (m) => m.user === deletePrompt.user,
      );
  ...
  const handleBanUser = async (userToBan: string) => {
    ...
    if (
      ...
      return showToast("Seul l'admin du groupe ou le Staff peut bannir.");
    if (userToBan === group.creator)
      return showToast("Impossible de bannir le créateur.");
    if ((group.subAdmins || []).includes(userToBan) && !isCreator)
      return showToast("Un sous-admin ne peut pas bannir un autre sous-admin.");
    ...
    try {
      ...
      if (userBanInfo.count >= 5) {
        return showToast(
          "Cet utilisateur est banni définitivement (5 renvois).",
        );
      }
  ...
  const handleDeleteDiscussion = async (
    ...
    if (
      !window.confirm(
        "Voulez-vous vraiment supprimer cette discussion définitivement pour tous ?",
      )
    )
      return;
    try {
  ...
  const handleToggleMute = async (userToMute: string) => {
    ...
    if (
      ...
      return showToast("Non autorisé.");
    if (userToMute === group.creator)
      return showToast("Impossible de couper le micro au créateur.");
  ...
  const handlePinGroup = async () => {
    ...
    try {
      ...
      if (currentUser?.pinnedGroups?.includes(activeGroup)) {
        newPinned = (currentUser.pinnedGroups || []).filter(
          (id) => id !== activeGroup,
        );
        await setDoc(
          userRef,
          { pinnedGroups: arrayRemove(activeGroup) },
          { merge: true },
        );
        ...
        await setDoc(
          userRef,
          { pinnedGroups: arrayUnion(activeGroup) },
          { merge: true },
        );
  ...
  const handleMarkAllAsRead = async () => {
    ...
    const nowISO = new Date().toISOString();
    const newTimestamps = {
      ...(state.currentUserData?.lastReadTimestamps || {}),
    };
  ...
  if (activeGroup) {
    ...
    const isAdmin =
      !isSMS &&
      ((group as Group).admins?.includes(state.currentUser as string) ||
        currentUser?.isAdmin ||
        currentUser?.isGrandAdmin ||
        currentUser?.isSuperAdmin);
    const isSubAdmin =
      !isSMS &&
      (group as Group).subAdmins?.includes(state.currentUser as string);
    const isCreator = !isSMS && (group as Group).creator === state.currentUser;
    const isMember =
      group.type === "public" ||
      (group.members || []).includes(state.currentUser as string);
    ...
    const currentAllowSpeak =
      pendingSettings.allowOthersToSpeak !== undefined
        ...
        : (group?.allowOthersToSpeak ?? true);
    const currentAllowInvite =
      pendingSettings.allowOthersToInvite !== undefined
        ? pendingSettings.allowOthersToInvite
    ...
    const canInvite =
      isSMS ||
      (group?.type === "private" &&
        (currentAllowInvite || isExemptFromGroupLimits));

    const handleDeleteSMS = async (idToDelete: string) => {
      try {
        const isSMSChat = idToDelete.startsWith("sms_");
        const path = isSMSChat ? "private_messages" : "groups";

        const msgsQuery = query(collection(db, path, idToDelete, "messages"));
        const msgsSnap = await getDocs(msgsQuery);
        for (const m of msgsSnap.docs) {
          await deleteDoc(doc(db, path, idToDelete, "messages", m.id));
        }
    ...
    return (
      <div
        className={`flex-1 flex flex-col h-full relative z-[2000] animate-in fade-in duration-300 bg-transparent ${state.darkMode ? "text-white" : "text-gray-900"}`}
      >
        {selectionMode ? (
          ...
          <div
            className={`p-4 border-b flex items-center justify-between shadow-sm z-[2001] sticky top-0 ${state.darkMode ? "bg-black/80 border-white/10 backdrop-blur-md text-white" : "bg-white"}`}
          >
            <div className="flex items-center gap-3">
              <button
                onClick={() => {
                ...
                className={`flex items-center gap-1.5 px-3 py-2 rounded-2xl transition-all shadow-sm border group ${state.darkMode ? "bg-zinc-800 border-white/10 text-white hover:bg-zinc-700" : "bg-gray-100 hover:bg-gray-200 text-gray-700 border-gray-200"}`}
              >
                <ChevronLeft
                  size={18}
              ...
              >
                {isSMS && otherUserData?.avatar ? (
                  ...
                  <span className={isSMS && state.darkMode ? "text-white" : ""}>
                    {(otherUser || group.name || "?")[0].toUpperCase()}
                  </span>
                )}
              </div>
              <div>
                <h3
                  className={`font-bold leading-tight flex items-center gap-2 ${state.darkMode ? "text-white" : "text-gray-800"}`}
                >
                  {isSMS ? otherUser : group.name}
                </h3>
                {!isSMS && group.type === "public" ? (
                  <p
                    className={`text-xs font-medium italic ${state.darkMode ? "text-gray-400" : "text-gray-500"}`}
                  >
                    Groupe Ouvert
                  </p>
                ) : isSMS ? (
                  <div className="flex flex-col xl:flex-row xl:items-center gap-2 mt-1.5">
                    {otherUserData?.lastSeen ? (
                      <div className={`flex items-center gap-2 px-2.5 py-1 rounded-lg border w-max ${state.darkMode ? "border-white/10 bg-white/5 shadow-inner" : "border-gray-200 bg-gray-100/50 shadow-sm"}`}>
                        <span className="text-[9px] font-black uppercase tracking-widest text-[#0D98BA]">Dernière Connexion</span>
                        <div className="flex gap-1.5 text-[9px] font-medium text-gray-500 dark:text-gray-400">
                          <span className={`flex items-center px-1.5 py-0.5 rounded shadow-sm border ${state.darkMode ? "bg-black/40 border-white/5" : "bg-white border-gray-100"}`}>
                            <b className="uppercase mr-1 font-black opacity-60">Jour:</b>{new Date(otherUserData.lastSeen).toLocaleDateString("fr-FR", { weekday: 'long', day: '2-digit', month: 'long', year: 'numeric' })}
                          </span>
                          <span className={`flex items-center px-1.5 py-0.5 rounded shadow-sm border ${state.darkMode ? "bg-black/40 border-white/5" : "bg-white border-gray-100"}`}>
                            <b className="uppercase mr-1 font-black opacity-60">Heure:</b>{new Date(otherUserData.lastSeen).toLocaleTimeString("fr-FR", { hour: '2-digit', minute: '2-digit', second: '2-digit' })}
                          </span>
                        </div>
                      </div>
                    ) : (
                      <div className={`flex items-center px-2.5 py-1 rounded-lg border w-max ${state.darkMode ? "border-white/10 bg-white/5 shadow-inner" : "border-gray-200 bg-gray-100/50 shadow-sm"}`}>
                        <span className="text-[9px] font-black uppercase tracking-widest text-gray-500">
                          {checkIsOnline(otherUserData) ? "Aucune donnée hors-ligne" : "Vu récemment"}
                        </span>
                      </div>
                    )}
                    <div className={`flex items-center gap-2 px-2.5 py-1 rounded-lg border w-max ${state.darkMode ? "border-white/10 bg-white/5 shadow-inner" : "border-gray-200 bg-gray-100/50 shadow-sm"}`}>
                      <span className="text-[9px] font-black uppercase text-gray-400 tracking-wider">Statut:</span>
                      <span className={`flex items-center gap-1.5 text-[10px] font-black uppercase tracking-wider ${checkIsOnline(otherUserData) ? "text-[#32CD32]" : "text-gray-500"}`}>
                        <span className={`w-2 h-2 rounded-full ${checkIsOnline(otherUserData) ? "bg-[#32CD32] animate-pulse shadow-[0_0_8px_rgba(50,205,50,0.6)]" : "bg-gray-400 border border-gray-300 dark:border-gray-600"}`}></span>
                        {checkIsOnline(otherUserData) ? "En ligne" : "Hors ligne"}
                      </span>
                    </div>
                  </div>
                ) : (
                  <p
                    className={`text-xs font-medium ${state.darkMode ? "text-gray-400" : "text-gray-500"}`}
                  >
            ...
            <div className="flex items-center gap-1">
              ...
              {!isTest && (
                <button
                  ...
                  className={`p-2 rounded-full transition ${isPinned ? djStyleText + " bg-blue-50" : "text-gray-400 hover:bg-gray-100"}`}
              ...
              {(isSMS || !isSMS) && (
                <button
                  ...
                  className={`p-2.5 ml-1 rounded-2xl transition-all ${state.darkMode ? "bg-zinc-800/80 text-white hover:bg-zinc-700" : "bg-gray-100 text-gray-500 hover:bg-gray-200"}`}
                  title="Voir l'icône"
                >
                  <ImageIcon size={20} />
                </button>
              ...
              {(isMember || isSMS) && (
                <button
                  ...
                  className={`p-2.5 ml-1 rounded-2xl transition-all ${showGroupSettings ? djStyleBg + " text-white shadow-lg" : state.darkMode ? "bg-zinc-800/80 text-white hover:bg-zinc-700" : "bg-gray-100 text-gray-500 hover:bg-gray-200"}`}
                  title={
                    isSMS
          ...
          <div className="bg-white border-b p-6 space-y-6 animate-in slide-in-from-top-4 max-h-[50vh] overflow-y-auto custom-scrollbar shrink-0">
            <div className="flex justify-between items-center">
              <h4 className="font-black uppercase tracking-widest text-xs text-gray-400">
                Paramètres de {isSMS ? "la discussion" : group.name}
              </h4>
              <div className="flex gap-2">
                      ...
                      onClick={() => {
                        ...
                        input.onchange = async (e: any) => {
                          ...
                          if (file.size > 5 * 1024 * 1024)
                            return showToast("Image trop grande (max 5Mo)");
                ...
                {!isCreator && !isSubAdmin && (
                  <span className="text-[10px] font-black uppercase text-blue-500 bg-blue-50 px-2 py-1 rounded-full">
                    Lecture seule
                  </span>
                )}
              </div>
            ...
            {!isSMS && group.type === "private" && canInvite && (
              <div className="bg-gray-50 p-4 rounded-2xl border border-gray-100">
                ...
                <code className="text-xl font-black tracking-[0.3em] text-[#0D98BA]">
                  {showCode ? group.code : "••••••••"}
                </code>
              </div>
            )}
                  ...
                  {Object.keys(state.users || {})
                    .filter(
                      (uid) =>
                        ...
                        ((state.users[uid].name?.toLowerCase() || "").includes(
                          smsSearch.toLowerCase(),
                        ) ||
                          smsSearch === ""),
                    )
                    .map((uid) => {
                      ...
                      return (
                        ...
                        >
                          <div className="flex items-center gap-2">
                            <button
                              onClick={() =>
                                updateState({ selectedUserModal: uid })
                              }
              ...
              <div className="space-y-4">
                ...
                <div className="max-h-60 overflow-y-auto space-y-2 custom-scrollbar">
                  {group.members
                    ...
                    .map((m: string) => {
                      ...
                      const isMemberSubAdmin = (group.subAdmins || []).includes(
                        m,
                      );
                      ...
                      return (
                        <div
                          key={m}
                          className="flex items-center justify-between p-3 bg-gray-50 rounded-2xl group/member"
                        >
                          <div className="flex items-center gap-3">
                            <button
                              onClick={() =>
                                updateState({ selectedUserModal: m })
                              }
                            ...
                            <div>
                              <div className="flex items-center gap-2">
                                ...
                                {isMemberCreator && (
                                  <span className="text-[8px] font-black uppercase px-2 py-0.5 bg-red-100 text-red-600 rounded-full">
                                    Admin du groupe
                                  </span>
                                )}
                                {isMemberSubAdmin && (
                                  <span className="text-[8px] font-black uppercase px-2 py-0.5 bg-blue-100 text-blue-600 rounded-full">
                                    Sous-Admin
                                  </span>
                                )}
                              </div>
                ...
                <label className="flex items-center justify-between cursor-pointer group">
                  ...
                  <div className="relative">
                    ...
                    />
                    <div
                      className={`block w-12 h-7 rounded-full transition-colors ${currentAllowSpeak ? "bg-[#32CD32]" : "bg-gray-300"} ${!isCreator && !isSubAdmin ? "opacity-50" : ""}`}
                    ></div>
                    <div
                      className={`dot absolute left-1 top-1 bg-white w-5 h-5 rounded-full transition-transform ${currentAllowSpeak ? "transform translate-x-5" : ""}`}
                    ></div>
                  </div>
                </label>
                ...
                <label className="flex items-center justify-between cursor-pointer group mt-4">
                  <div className="flex items-center gap-2">
                    <span className="text-sm font-bold text-gray-700">
                      Autoriser les autres à ajouter des membres
                    </span>
                    {hasInviteChanged && (
                      <button
                        onClick={async (e) => {
                  ...
                  <div className="relative">
                    ...
                    />
                    <div
                      className={`block w-12 h-7 rounded-full transition-colors ${currentAllowInvite ? "bg-[#32CD32]" : "bg-gray-300"} ${!isCreator && !isSubAdmin ? "opacity-50" : ""}`}
                    ></div>
                    <div
                      className={`dot absolute left-1 top-1 bg-white w-5 h-5 rounded-full transition-transform ${currentAllowInvite ? "transform translate-x-5" : ""}`}
                    ></div>
                  </div>
                </label>
              </div>
        ...
        <div className="flex-1 overflow-y-auto px-4 lg:px-8 py-6 custom-scrollbar w-full">
          {state.activeThreadId && (
            <div className="mb-6 flex items-center justify-between p-4 bg-[#0D98BA]/5 border border-[#0D98BA]/10 rounded-[2rem] animate-in slide-in-from-top-4 duration-300">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h3 className="text-sm font-black uppercase tracking-widest text-[#0D98BA]">
                    Fil de discussion
                  </h3>
                  <p className="text-[10px] font-bold text-gray-400 uppercase tracking-tighter">
                    {state.activeThreadName}
                  </p>
                </div>
              </div>
              <div className="flex items-center gap-2">
          ...
          <div className="flex flex-col gap-2 w-full">
            {(group.messages || [])
              .filter(
                (msg) =>
                  ...
                  (state.users && state.users[msg.user]),
              )
              .filter((msg) => {
                if (state.activeThreadId) {
              ...
              .map((msg, idx, arr) => {
                ...
                const isGroupAdmin =
                  !isSMS &&
                  msg.user !== "dj-bot" &&
                  ((group as Group).creator === msg.user ||
                    (group as Group).subAdmins?.includes(msg.user));

                const isDeletedForMe = msg.deletedForUsers?.includes(
                  state.currentUser as string,
                );
                ...
                if (msg.isSystem) {
                  return (
                    <div key={msg.id} className="flex justify-center my-2">
                      <span className="bg-gray-200/80 text-gray-600 text-[10px] px-4 py-1 rounded-full uppercase font-bold shadow-sm">
                        {msg.text}
                      </span>
                    </div>
                  );
                ...
                return (
                  <div
                    ...
                    className={`flex ${isMine ? "flex-row-reverse" : "flex-row"} items-end gap-1.5 group/msg ${isSameSender ? "mt-0.5" : "mt-3"} ${selectionMode ? "cursor-pointer hover:bg-gray-100/50 p-1 rounded-xl transition-colors" : ""} ${isSelected ? "bg-blue-50/50 outline outline-2 outline-blue-200" : ""}`}
                  >
                    {selectionMode && (
                      <div className="shrink-0 mr-1 self-center">
                        <div
                          className={`w-5 h-5 rounded-full border-2 flex items-center justify-center transition-colors ${isSelected ? "bg-[#0D98BA] border-[#0D98BA] text-white" : "border-gray-300"}`}
                        >
                    ...
                    <button
                      onClick={() => {
                        if (
                          !isDeletedAccount &&
                          msg.user !== "dj-bot" &&
                          state.users?.[msg.user]
                        )
                          updateState({ selectedUserModal: msg.user });
                      }}
                      disabled={
                        isDeletedAccount ||
                        msg.user === "dj-bot" ||
                    ...
                    >
                      ...
                      <div
                        className={`flex items-center gap-1.5 ${isMine ? "flex-row-reverse" : "flex-row"} max-w-full`}
                      >
                        <div
                          className={`relative px-2.5 py-1.5 rounded-2xl shadow-sm ${isMine ? `rounded-tr-none text-white ${djStyleBg}` : state.darkMode ? "rounded-tl-none bg-zinc-800 border-white/10 text-white" : "rounded-tl-none bg-white text-gray-800"} 
                  ...
                  `}
                        >
                          {(isStaff || isGroupAdmin) && (
                            <div
                              className={`flex gap-1 mb-1.5 ${isMine ? "justify-end" : "justify-start"}`}
                            >
                              {isStaff && (
                                <span className="text-[7.5px] font-black uppercase tracking-widest text-blue-500 bg-blue-50 px-1.5 py-0.5 rounded-sm">
                                  Staff App
                                </span>
                              )}
                              {isGroupAdmin && (
                                <span className="text-[7.5px] font-black uppercase tracking-widest text-green-600 bg-green-50 px-1.5 py-0.5 rounded-sm shrink-0">
                                  Admin Groupe
                                </span>
                              )}
                            </div>
                          ...
                          !(isDeletedForEveryone && !isRevealed) ? (
                            <div className="mb-2 space-y-2 min-w-[200px]">
                              ...
                              {(msg.files || []).map((file, idx) => (
                                <div
                                  key={idx}
                                  className={`rounded-xl overflow-hidden shadow-inner relative group/file ${state.darkMode ? "bg-zinc-800/50" : "bg-gray-50"}`}
                                >
                                  {file.type === "image" ||
                                  file.type === "sticker" ? (
                                    <img
                                    ...
                                    <div
                                      className={`p-4 flex flex-col gap-2 rounded-xl transition-colors ${state.darkMode ? "bg-zinc-800/80 text-white" : "bg-gray-100 text-gray-900"}`}
                                    >
                                      <div className="flex items-center gap-3">
                                    ...
                                    <div
                                      className={`p-4 flex items-center gap-4 rounded-xl transition-colors cursor-pointer ${state.darkMode ? "bg-zinc-800/80 hover:bg-zinc-700 text-white" : "bg-gray-100 hover:bg-gray-200 text-gray-900"}`}
                                      onClick={() =>
                                        handleDownload(
                                          file.url,
                                          file.name || "file",
                            ...
                            msg.fileUrl && (
                              <div
                                className={`mb-2 rounded-xl overflow-hidden shadow-inner relative group/file ${state.darkMode ? "bg-zinc-800/50" : "bg-gray-50"}`}
                              >
                                {msg.fileType === "image" ||
                            ...
                            !(isDeletedForEveryone && !isRevealed) && (
                              <div className="mb-3 p-5 bg-[#0a0a0a] rounded-[2rem] border border-white/5 space-y-4 shadow-2xl overflow-hidden relative">
                                ...
                                <div className="flex justify-between items-start gap-2">
                                  <h4 className="font-black text-sm uppercase tracking-tighter text-white leading-tight">
                                    {msg.poll.question}
                                  </h4>
                                  {msg.poll.closed && (
                                    <span className="text-[8px] font-black uppercase px-2.5 py-1 bg-red-500 text-white rounded-full shadow-lg">
                                      Clôturé
                                    </span>
                                  )}
                                </div>
                          ...
                          <p
                            className={`text-sm break-words whitespace-pre-wrap leading-relaxed ${isDeletedForEveryone && !isRevealed ? "italic opacity-60" : ""}`}
                          >
                            {isDeletedForEveryone && !isRevealed ? (
                              <span
                                className={
                        ...
                        {!isDeletedAccount && !selectionMode && (
                          <div
                            className={`flex items-center gap-1 z-10 opacity-100 transition-opacity duration-200 bg-transparent shadow-none whitespace-nowrap self-center ${isMine ? "flex-row-reverse" : "flex-row"}`}
                          >
                            {(isAdmin || isCreator || isSubAdmin || isMine) && (
                              ...
                              >
                                <Trash2 size={14} />
                                <div className="absolute bottom-full mb-1 left-1/2 -translate-x-1/2 px-2 py-1 bg-gray-800 text-white text-[8px] font-black uppercase rounded opacity-0 group-hover/btn:opacity-100 transition-opacity whitespace-nowrap pointer-events-none shadow-sm z-50">
                                  Supprimer
                                </div>
                              </button>
                            )}
                      ...
                      <div className="flex items-center mt-0.5">
                        <span className="text-[9px] text-gray-400 mx-1 font-medium">
                          {msg.timestamp
                        ...
                        {msg.isPending && (
                          <span
                            className="ml-1 text-gray-400 opacity-60 flex items-center"
                            title="En attente de réseau (file d'attente)"
                          >
            ...
            <div
              className={`transition-opacity duration-300 px-6 py-2 ${typingNames.length > 0 ? "opacity-100" : "opacity-0 h-0 overflow-hidden"}`}
            >
              <div className="flex items-center gap-2 px-3 py-1.5 bg-[#0D98BA]/10 rounded-2xl w-fit border border-[#0D98BA]/20 shadow-sm">
                <div className="flex gap-1">
                  <div
                    className="w-1.5 h-1.5 bg-[#0D98BA] rounded-full animate-bounce"
            ...
            {uploadJobs.length > 0 && (
              <div className="flex flex-col gap-2 px-6 py-2">
                {uploadJobs.map((job) => (
                  <div key={job.id} className="flex justify-end mb-2">
                    <div
                      className={`relative max-w-[85%] rounded-[1.5rem] rounded-br-[0.375rem] p-4 shadow-md ${state.darkMode ? "bg-zinc-800 text-white" : "bg-gray-100 text-gray-900 border border-gray-200"} opacity-70`}
                    >
                      <div className="flex items-center justify-between gap-4 mb-2">
                        ...
                        {job.xhr && (
                          <button
                            onClick={() => job.xhr?.abort()}
                            className="bg-red-50 hover:bg-red-100 text-red-500 rounded-full p-1.5 transition-colors cursor-pointer"
                      ...
                      <div className="w-full bg-gray-300/30 rounded-full h-1.5 overflow-hidden mb-1">
                        <div
                          className="h-full transition-all duration-300 bg-[#0D98BA]"
                          style={{ width: `${job.progress}%` }}
                        />
                      </div>
                      <div className="flex justify-between items-center text-[9px] font-black uppercase tracking-widest text-[#0D98BA] mt-1">
        ...
        <div
          className={`p-3 border-t shadow-[0_-4px_6px_-1px_rgba(0,0,0,0.05)] z-10 backdrop-blur-md ${state.darkMode ? "bg-black/90 border-white/10" : "bg-white border-gray-100"}`}
        >
          {!isMember && group.type === "private" && !isSMS ? (
            <div className="flex flex-col items-center gap-3 p-4 bg-gray-50 rounded-2xl border border-gray-100">
              ...
              <p className="text-xs text-gray-400 italic">
                Vous êtes en lecture seule sur ce groupe privé.
              </p>
            </div>
          ) : (
            <form
              ...
              <textarea
                ...
                placeholder={
                  isTest
                    ...
                    : state.activeThreadId
                      ? `Écrire dans le fil "${state.activeThreadName}"...`
                      : !isMember && group.type === "private" && !isSMS
                        ? "Mode Secret - Lecture seule"
                        : !canSpeak
                          ? "Seuls les admins peuvent parler ici."
        ...
        {showDeleteSmsPrompt && (
          <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-[10000] flex items-center justify-center p-4 animate-in fade-in duration-200">
            <div className="bg-white rounded-[2rem] p-8 max-w-sm w-full shadow-2xl animate-in zoom-in-95 duration-300">
              ...
              <div className="flex gap-3">
                <button
                  onClick={() => setShowDeleteSmsPrompt(null)}
                  className="flex-1 py-3 rounded-xl font-bold text-gray-700 bg-gray-100 hover:bg-gray-200 transition"
                >
                  Annuler
                </button>
                <button
                  onClick={() => handleDeleteSMS(showDeleteSmsPrompt)}
                  className="flex-1 py-3 rounded-xl font-bold text-white bg-red-500 hover:bg-red-600 transition shadow-lg shadow-red-500/30"
                >
                  Supprimer
                </button>
              </div>
          ...
          <div className="fixed inset-0 bg-black/40 backdrop-blur-sm z-[5000] flex items-center justify-center p-4 animate-in fade-in">
            <div className="bg-white rounded-[2.5rem] w-full max-w-sm overflow-hidden shadow-2xl">
              ...
              <div className="p-6 space-y-3">
                <button
                  onClick={() =>
                    handleDeleteMessage(deleteOptionsPrompt.msgIds, "me")
                  }
          ...
          <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-[5000] flex items-center justify-center p-4 animate-in fade-in">
            <div className="bg-white rounded-3xl p-6 max-w-sm w-full shadow-2xl animate-in zoom-in-95">
              ...
              <p className="text-gray-600 mb-6">
                Voulez-vous supprimer tous les messages de ce compte supprimé,
                ou seulement ce message ?
              </p>
              <div className="flex flex-col gap-3">
                ...
                <button
                  ...
                  className="w-full py-3 rounded-xl font-bold text-red-500 bg-red-50 hover:bg-red-100 transition"
                >
                  Supprimer uniquement ce message
                </button>
                <button
                  onClick={() => setDeletePrompt(null)}
                  className="w-full py-3 rounded-xl font-bold text-gray-600 bg-gray-100 hover:bg-gray-200 transition mt-2"
                >
                  Annuler
                </button>
              </div>
        ...
        {staffDeletePrompt && (
          <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-[5000] flex items-center justify-center p-4 animate-in fade-in">
            <div className="bg-white rounded-3xl p-6 max-w-sm w-full shadow-2xl animate-in zoom-in-95">
              ...
              <p className="text-gray-600 mb-6">
                En tant que membre du staff, voulez-vous supprimer ce message
                pour TOUS les utilisateurs ? Cette action est irréversible.
              </p>
              <div className="flex gap-3">
                <button
                  onClick={() => setStaffDeletePrompt(null)}
        ...
        {inputPrompt && (
          <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-[10000] flex items-center justify-center p-4 animate-in fade-in">
            <div className="bg-white rounded-3xl p-6 max-w-sm w-full shadow-2xl animate-in zoom-in-95">
              <h3 className="text-xl font-black uppercase tracking-tighter text-gray-800 mb-4">
                {inputPrompt.message}
              </h3>
              <input
                type="text"
                ...
                onChange={(e) =>
                  setInputPrompt({ ...inputPrompt, value: e.target.value })
                }
              ...
              <div className="flex gap-2">
                <button
                  ...
                  className="flex-1 py-3 rounded-xl font-bold bg-gray-100 hover:bg-gray-200 transition"
                >
                  Annuler
                </button>
                <button
                  ...
                  className="flex-1 py-3 rounded-xl font-bold text-white bg-[#0D98BA] hover:opacity-90 transition"
                >
                  Valider
                </button>
              </div>
  ...
  return (
    <div
      className={`flex flex-col h-full animate-in fade-in duration-300 bg-transparent ${state.darkMode ? "text-white" : "text-gray-900"}`}
    >
      ...
      <div className="flex-1 overflow-y-auto px-6 pb-6">
        {activeTab === "sms" && (
          <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4">
            ...
            <div
              className={`p-4 rounded-3xl shadow-md border mb-4 flex justify-between items-center ${state.darkMode ? "bg-zinc-900/50 border-white/5" : "bg-white border-gray-50"}`}
            >
            ...
            <div className="space-y-4">
                  ...
                  {Object.keys(state.users || {})
                    ...
                    .map((uid) => {
                      ...
                      return (
                        <div
                          key={uid}
                          className={`flex items-center justify-between p-4 rounded-2xl shadow-sm border transition-all ${state.darkMode ? "bg-zinc-800/50 border-white/10" : "bg-white border-gray-50"}`}
                        >
                          <div className="flex items-center gap-4">
                            <div
                              className="w-12 h-12 rounded-xl bg-gray-100 flex items-center justify-center font-black text-gray-400 shadow-inner overflow-hidden cursor-pointer"
                              onClick={() =>
                                updateState({ selectedUserModal: uid })
                              }
                            >
                            ...
                            <div>
                              ...
                              <div
                                className={`mt-3 w-full sm:w-max sm:min-w-[220px] grid grid-cols-1 gap-y-2 text-[10px] p-3 rounded-2xl shadow-sm border backdrop-blur-sm ${state.darkMode ? "bg-zinc-900/40 border-white/5" : "bg-gray-100/50 border-gray-200/50"}`}
                              >
                                <div className="flex flex-col">
                                  <span className="text-gray-400 font-extrabold uppercase tracking-widest text-[8px] mb-0.5">
                                    Dernière co.
                                  </span>
                                  <span
                                    className={`${state.darkMode ? "text-gray-200" : "text-gray-700"} font-bold`}
                                  >
                                    {u.lastLogin
                                ...
                                <div
                                  className={`flex items-center justify-between border-t mt-1 pt-2 ${state.darkMode ? "border-white/5" : "border-gray-200/50"}`}
                                >
                                  <span className="text-gray-400 font-extrabold uppercase tracking-widest text-[8px]">
                                    Statut:
                                  </span>
                                  <span
                                    className={`flex items-center gap-1.5 text-[10px] font-bold ${checkIsOnline(u) ? "text-green-500" : "text-gray-500"}`}
                                  >
                                    <span
                                      className={`w-1.5 h-1.5 rounded-full ${checkIsOnline(u) ? "bg-green-500 animate-pulse shadow-[0_0_5px_rgba(34,197,94,0.5)]" : "bg-gray-400"}`}
                                    ></span>
                                    {checkIsOnline(u)
                                      ? "En ligne"
                                      : "Hors ligne"}
              ...
              {visibleSMS.length > 0 && (
                <h3 className="text-[10px] font-black uppercase tracking-[0.2em] text-gray-400 mt-6 mb-4">
                  Vos discussions SMS en cours
                </h3>
              )}
              {visibleSMS.map((chat, i) => {
                const otherId = chat.members?.find(
                  (m: string) => m !== state.currentUser,
                );
                const otherName =
                  state.users[otherId || ""]?.name || otherId || "Inconnu";
                const lastMsg =
                ...
                return (
                  ...
                  >
                    <div className="w-14 h-14 rounded-full bg-gray-100 flex items-center justify-center text-gray-500 font-bold text-xl shadow-inner group-hover:scale-105 transition-transform overflow-hidden">
                      {state.users[otherId || ""]?.avatar ? (
                        <img
                          src={state.users[otherId || ""].avatar!}
                          className="w-full h-full object-cover"
                        />
                      ) : (
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center justify-between mb-1">
                        <h3
                          className={`font-bold truncate pr-2 flex items-center gap-2 ${state.darkMode ? "text-white" : "text-gray-900"}`}
                        >
                          {otherName}
                          {checkIsOnline(state.users[otherId || ""]) && (
                      ...
                      <p className="text-sm text-gray-500 truncate">
                        {lastMsg
                          ? (lastMsg.user === state.currentUser
                              ? "Vous: "
                              : "") + lastMsg.text
              ...
              >
                {item.newCount > 0 && (
                  <div className="absolute top-0 right-0 bg-[#0D98BA] text-white text-[10px] font-black px-3 py-1 rounded-bl-xl shadow-md animate-pulse">
                    {item.newCount} nouveau{item.newCount > 1 ? "x" : ""}{" "}
                    message{item.newCount > 1 ? "s" : ""}
                  </div>
                )}
                <div className="flex justify-between items-center mb-2">
                  <div className="flex items-center gap-2">
                    <span
                      className={`text-[10px] font-black uppercase tracking-widest ${djStyleText}`}
                    >
                      {item.groupName}
                    </span>
                ...
                <div className="flex items-center gap-3">
                  <div className="w-10 h-10 rounded-xl bg-gray-100 flex items-center justify-center text-xs font-bold text-gray-500 shrink-0 overflow-hidden shadow-inner">
                    {item.avatar ? (
                      ...
                      <div
                        className={`w-full h-full flex items-center justify-center ${item.isSMS ? "bg-gray-100" : "bg-gradient-to-br from-[#007FFF] to-[#32CD32] text-white"}`}
                      >
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-1.5 mb-0.5">
                      <p
                        className={`text-xs font-bold ${state.darkMode ? "text-zinc-300" : "text-gray-700"}`}
                      >
                        @
                        {item.lastMessage.user === "test"
                          ? "Anonyme"
                          : state.users[item.lastMessage.user]?.name ||
                            item.lastMessage.user}
                      </p>
                      {item.type === "sms" &&
                        checkIsOnline(state.users[item.lastMessage.user]) && (
                          <div className="w-1.5 h-1.5 rounded-full bg-green-500 shadow-[0_0_8px_rgba(34,197,94,0.6)]"></div>
                        )}
                    </div>
                    <p
                      className={`text-sm truncate ${item.newCount > 0 ? (state.darkMode ? "text-white font-bold" : "text-gray-900 font-bold") : "text-gray-500"}`}
                    >
          ...
          <div className="space-y-6">
              ...
              <div className="flex flex-col gap-4">
                <div className="flex justify-between items-center mb-2">
                  ...
                  <button
                    ...
                    className={`p-2 rounded-full ${djStyleBg} shadow-lg hover:scale-110 transition-transform`}
                  >
                    <Plus size={16} className="text-white" />
                  </button>
                </div>
                {visibleGroups.map((g, i) => {
                  const isMember = (g.members || []).includes(
                    state.currentUser as string,
                  );
                  return (
                    <div
                      ...
                      className={`flex items-center gap-4 p-4 rounded-2xl shadow-sm border cursor-pointer hover:shadow-md transition-all group relative ${state.darkMode ? "bg-zinc-800/50 border-white/10 hover:border-[#0D98BA]" : "bg-white border-gray-100 hover:border-blue-100"}`}
                    >
                      ...
                      <div
                        className={`w-14 h-14 rounded-2xl flex items-center justify-center text-white font-bold text-xl shadow-inner group-hover:scale-105 transition-transform overflow-hidden ${g.avatar ? "bg-white" : "bg-gradient-to-br from-[#007FFF] to-[#32CD32]"}`}
                      >
                        {g.avatar ? (
                          <img
                            src={g.avatar}
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center justify-between mb-1">
                          <h3
                            className={`font-bold truncate pr-2 ${state.darkMode ? "text-white" : "text-gray-900"}`}
                          >
                            {g.name}
                          </h3>
                          <span className="text-xs text-gray-400 whitespace-nowrap">
                        ...
                        <div className="flex items-center justify-between">
                          <p
                            className={`text-sm truncate ${state.newMessages?.includes(g.id) ? (state.darkMode ? "text-white font-bold" : "text-gray-900 font-bold") : "text-gray-500"}`}
                          >
                            {g.messages && g.messages.length > 0
            ...
            {activeTab === "private" && (
              <div className="flex flex-col gap-4">
                ...
                <div
                  className={`p-6 rounded-3xl shadow-xl border mb-4 ${state.darkMode ? "bg-zinc-900 border-white/5" : "bg-white border-gray-50"}`}
                >
                  <h4 className="text-xs font-black uppercase tracking-widest text-gray-400 mb-4">
                    Intégrer un groupe via le code
                  </h4>
                  <div className="flex gap-2">
                    <input
                      type="text"
                    ...
                    <button
                      ...
                      className={`px-6 py-3 rounded-xl font-black uppercase tracking-widest text-xs shadow-lg hover:opacity-90 transition active:scale-95 text-white ${djStyleBg}`}
                    >
                      Rejoindre
                    </button>
                  </div>
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center justify-between mb-1">
                        <div className="flex items-center gap-2 min-w-0">
                          <h3
                            className={`font-bold truncate ${state.darkMode ? "text-white" : "text-gray-900"}`}
                          >
                            {g.name}
                          </h3>
                          {activeTab === "private" &&
                            !g.members?.includes(state.currentUser) && (
                              <span className="text-[8px] font-black uppercase px-2 py-0.5 bg-purple-100 text-purple-600 rounded-full shrink-0">
                                Ghost
                              </span>
                            )}
                        </div>
      ...
      {deletePrompt && (
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-[5000] flex items-center justify-center p-4 animate-in fade-in">
          <div className="bg-white rounded-3xl p-6 max-w-sm w-full shadow-2xl animate-in zoom-in-95">
            ...
            <p className="text-gray-600 mb-6">
              Voulez-vous supprimer tous les messages de ce compte supprimé, ou
      ...
      {staffDeletePrompt && (
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-[5000] flex items-center justify-center p-4 animate-in fade-in">
          <div className="bg-white rounded-3xl p-6 max-w-sm w-full shadow-2xl animate-in zoom-in-95">
            ...
            <p className="text-gray-600 mb-6">
              En tant que membre du staff, voulez-vous supprimer ce message pour
              TOUS les utilisateurs ? Cette action est irréversible.
            </p>
            <div className="flex gap-3">
              <button
                onClick={() => setStaffDeletePrompt(null)}
